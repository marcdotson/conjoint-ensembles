...
- When ANA is present, simulate ANA where each respondent pays attention to at least one attribute and has non-attendance for at least one attribute.
- When screening is present, simulate screening where each respondent screens based on one attribute level.
- When respondent quality is present, force a random choice for up to 25 respondents.
- Generate betas as a random normal deviation from a random uniform baseline from -1 to 2.
- When generating betas, multiply by ANA indicator and add screening...does this apply regardless of flag??
