...
We will present the theoretical justification for our approach and practical strategies for implementation.
...
Respondents come with priorities, which manifest themselves in the way respondents screen alternatives. Within conjoint, alternatives that pass a respondent's 'screen' are evaluated; alternatives that do not pass the screen will not be evaluated. They might have a minimum requirements that must be met (conjunctive screening rule), or missing features must be made up for by including new features (disjunctive), or they come to a decision by considering all of the attributes and by adding the positive and negative.   Models have also been constructed to account for conjunctive, disjunctive, and compensatory screening rules (Gilbride & Allenby 2004).
...
We could choose the base models a different way. For example, each base models could be specified with a different prior that accounts for a specific pathology or respondent behavior. Then the base models will generate predictions that account for the effects of each pathology respectively. Other ways of specifying base models include dropping individuals or choice-tasks from the design matrix. In this paper, we show results for the ensemble generated by base models trained on $l-1$ feature variables.
